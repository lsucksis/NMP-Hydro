...
namespace NoahMP
{
	...
	public class NoahMP
	{
		...
		/// </summary>
		//public static double BEXP;
		/// <summary>
		/// dry soil moisture threshold where direct evap from top layer ends (volumetric) ( Assigned in REDPRM )
		/// </summary>
		//		public static double SMCDRY;
		/// <summary>
		...
		/// <param name="QPRECC"></param>
		/// <param name="QPRECL"></param>
		/// <param name="SOLAD"></param>
		/// <param name="SOLAI"></param>
		/// <param name="SWDOWN"></param>
			...
			out double SWDOWN)
		{
			...
			THAIR = SFCTMP * Math.Pow(SFCPRS / PAIR, NoahMP.RAIR / NoahMP.CPAIR);
			...
			RHOAIR = (SFCPRS - 0.378 * EAIR) / (NoahMP.RAIR * SFCTMP);
...
//			if(Math.Abs( ALBEDO-0.41345)<1e-6)
//			{
//			    Console.WriteLine(ALBEDO);
//			}
...
//					Console.WriteLine("ZWT=" + ZWT.ToString("e") + "\tWA=" + WA.ToString("e") + "\tWT=" + WT.ToString("e") + "\tQIN=" + QIN.ToString("e") + "\tQDIS=" + QDIS.ToString("e"));
		...
		/// <param name="ILOC"></param>
		/// <param name="JLOC"></param>
		/// <param name="FROZEN_CANOPY"></param>
		/// <param name="CANLIQ"></param>
		/// <param name="CANICE"></param>
		/// <param name="TV"></param>
...
//			Console.WriteLine("QRAIN:"+QRAIN+" ECAN"+ECAN);
		...
		/// <summary>
		/// 2024已检查代码，发现有许多刘永和自己设计的修改，之前应该已测试过
		/// </summary>
		/// <param name="VEGTYP"></param>
		/// <param name="ISURBAN"></param>
...
//				throw new Exception();
		...
		/// <param name="SNICEV"></param>
		/// <param name="SNLIQV"></param>
		/// <param name="EPORE"></param>
		/// <param name="T2M"></param>
		/// <param name="FSNO"></param>
		/// <param name="SAV"></param>
		/// <param name="SAG"></param>
		...
		/// <param name="frozen_canopy"></param>
		/// <param name="frozen_ground"></param>
		/// <param name="TV"></param>
		/// <param name="TG"></param>
		/// <param name="STC"></param>
		/// <param name="SNOWH"></param>
			...
			if (IST == 1) {
				...
				for (int IZ = 1; IZ <= NROOT; IZ++) {					
					...
					if (NoahMP.OPT_BTR == 2) {                   // CLM
						...
						GX = (1 - PSI / PSIWLT) / (1 + cell.PSISAT / PSIWLT);
					}
...
//			}
		...
		/// </summary>
		/// <param name="DZ"></param>
		/// <param name="WLIQ"></param>
		/// <param name="WICE"></param>
		/// <param name="T"></param>
		/// <param name="DZ2"></param>
		/// <param name="WLIQ2"></param>
		/// <param name="WICE2"></param>
		/// <param name="T2"></param>
...
//			double EPORE = 0; //effective porosity [m3/m3]
			...
			if (NoahMP.OPT_RUN == 3) {
				//不更新SH2O
				WATER_Input.INFIL(cell, NSOIL, DT, ZSOIL, SH2O, SICE,
...
//					throw new Exception();
		...
		/// <param name="SFCTMP"></param>
		/// <param name="SFCPRS"></param>
		/// <param name="Q2SAT"></param>
		/// <param name="DQSDT2"></param>
...
//			IRG = 0;
			...
			if ((HCAN - ZPD) <= 0) {
				...
				// 	          wrf_message ( message )
				//          wrf_error_fatal ( "CRITICAL PROBLEM IN MODULE_SF_NOAHMPLSM:VEGEFLUX" )
			}
			...
			for (int ITER = 1; ITER <= NITERC; ITER++) {
				if (ITER == 1) {
					Z0H = Z0M;
					Z0HG = Z0MG;
				} else {
...
//				}
			...
			for (int ITER = 1; ITER <= NITERG; ITER++) {
				T = TDC(TG);
				ESAT(T, out ESATW, out ESATI, out DSATW, out DSATI);
				if (T > 0) {
					ESTG = ESATW;
			...
			if (NoahMP.OPT_STC == 1) {
				if (SNOWH > 0.05 && TG > NoahMP.TFRZ) {
					TG = NoahMP.TFRZ;
					double TG4=TG*TG*TG*TG;
					TV4=TV*TV*TV*TV;
					IRG = CIR * TG4 - EMG * (1 - EMV) * LWDN - EMG * EMV * NoahMP.SB * TV4;
					SHG = CSH * (TG - TAH);
					EVG = CEV * (ESTG * RHSUR - EAH);
					GH = SAG - (IRG + SHG + EVG);
				}
...
//						throw new Exception("");
			...
			out double  FSRG, out double BGAP, out double WGAP)
		{
			...
			SURRAD(MPE, FSUN, FSHA, ELAI, VAI,
				...
				out FSR,                                //  //out
				FREVI, FREVD, FREGD, FREGI, out FSRV, // //inout
				out FSRG);
		}
		/// <summary>
		/// 2024已检查
		...
		/// <param name="FSUN"></param>
		/// <param name="FSHA"></param>
		/// <param name="ELAI"></param>
		/// <param name="VAI"></param>
		/// <param name="LAISUN"></param>
		/// <param name="LAISHA"></param>
		/// <param name="SOLAD"></param>
		/// <param name="SOLAI"></param>
		/// <param name="FABD"></param>
		/// <param name="FABI"></param>
		/// <param name="FTDD"></param>
		/// <param name="FTID"></param>
		/// <param name="FTII"></param>
		/// <param name="ALBGRD"></param>
		/// <param name="ALBGRI"></param>
		/// <param name="ALBD"></param>
		/// <param name="ALBI"></param>
		/// <param name="ILOC"></param>
		/// <param name="JLOC"></param>
		/// <param name="PARSUN"></param>
		/// <param name="PARSHA"></param>
		/// <param name="SAV"></param>
		/// <param name="SAG"></param>
		/// <param name="FSA"></param>
		/// <param name="FSR"></param>
		/// <param name="FREVI"></param>
		/// <param name="FREVD"></param>
		/// <param name="FREGD"></param>
		/// <param name="FREGI"></param>
		/// <param name="FSRV"></param>
		/// <param name="FSRG"></param>
		public static	void SURRAD(double MPE, double FSUN, double FSHA, double ELAI, double VAI,
			...
			out double FSR,                                //       //out
			double[]  FREVI, double[] FREVD, double[] FREGD, double[] FREGI, out double FSRV,
			out double  FSRG)
		{
...
//			 REAL :: CEA         //constrain ea or else model blows up
			...
			double FNF = Math.Min(FOLN / Math.Max(MPE, Driver.vegparams.FOLNMX[VEGTYP - 1]), 1.0);
			...
			// first guess ci			
			double CI = 0.7 * CO2 * C3PSN + 0.4 * CO2 * (1.0 - C3PSN);
			// rb: s/m -> s m**2 / umol
			...
			for (int ITER = 1; ITER <= NITER; ITER++) {
				...
				double R2 = C / Q;
				RS = Math.Max(R1, R2);
				CI = Math.Max(CS - PSN * SFCPRS * 1.65 * RS, 0);
			}
...
//			Console.WriteLine("PSN="+PSN+" XLAI="+XLAI+" XSAI="+XSAI);
		...
		/// <param name="SNLIQ"></param>
		/// <param name="DZSNSO"></param>
		/// <param name="TKSNO"></param>
		/// <param name="CVSNO"></param>
		/// <param name="SNICEV"></param>
		/// <param name="SNLIQV"></param>
		/// <param name="EPORE"></param>
		public static void CSNOW(int ISNOW, int NSNOW, int NSOIL, FortDoubleArray SNICE, FortDoubleArray SNLIQ, FortDoubleArray DZSNSO, FortDoubleArray TKSNO, FortDoubleArray CVSNO, FortDoubleArray SNICEV, FortDoubleArray SNLIQV, FortDoubleArray EPORE)
		{
			...
			for (int IZ = ISNOW + 1; IZ <= 0; IZ++) {
				...
				SNICEV[IZ] = Math.Min(1, SNICE[IZ] / (DZSNSO[IZ] * NoahMP.DENICE));
				EPORE[IZ] = 1 - SNICEV[IZ];
				SNLIQV[IZ] = Math.Min(EPORE[IZ], SNLIQ[IZ] / (DZSNSO[IZ] * NoahMP.DENH2O));
			}
...
//	throw new Exception("");
		...
		/// <param name="FREV"></param>
		/// <param name="FREG"></param>
		/// <param name="BGAP"></param>
		/// <param name="WGAP"></param>
...
//
		...
		/// </summary>
		/// <param name="NSOIL"></param>
		/// <param name="NBAND"></param>
		/// <param name="ICE"></param>
		/// <param name="IST"></param>
		/// <param name="ISC"></param>
		/// <param name="FSNO"></param>
		/// <param name="SMC"></param>
		/// <param name="ALBSND"></param>
		/// <param name="ALBSNI"></param>
		/// <param name="COSZ"></param>
		/// <param name="TG"></param>
		/// <param name="ILOC"></param>
		/// <param name="JLOC"></param>
